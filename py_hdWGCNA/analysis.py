...
def find_dmes(adata: AnnData,
    ...
    me_key_map = {
        'hMEs': 'hMEs', 'MEs': 'MEs',
        'scores': 'module_scores', 'average': 'avg_module_expr'
    }
    ...
    if n1 < 3 or n2 < 3:
        raise ValueError(f"Insufficient cells: {group1}={n1}, {group2}={n2} (min 3 each)")
...
def find_all_dmes(adata: AnnData,
                  group_by: str,
                  features: str = 'hMEs',
                  test: str = 'wilcox',
                  logfc_threshold: float = 0.25,
    ...
    for i, g1 in enumerate(groups):
        for g2 in groups[i+1:]:
            ...
            try:
                dme_res = find_dmes(
                    adata, group_by=group_by, group1=g1, group2=g2,
...
def module_trait_correlation(adata: AnnData,
                              trait_cols: List[str] = None,
                              features: str = 'hMEs',
                              method: str = 'pearson',
                              wgcna_name: str = None) -> Optional[Dict]:
    """
    ...
    for tcol in trait_cols:
        ...
        for mod in MEs_clean.columns:
            ...
            r, p = stats.pearsonr(
                me_vals[both_valid], trait_clean[both_valid]
