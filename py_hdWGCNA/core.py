...
def setup_for_wgcna(
    ...
    print(
        f"SetupForWGCNA complete: {len(genes_use)} genes selected using '{gene_select}' method"
    )
    adata = set_dat_expr(
        adata,
        group_by=None,
...
def set_dat_expr(
    ...
    if use_metacells and "metacell_obj" in wgcna_data:
        ...
        if (
            subset_col is not None
            and subset_col in mc_adata.obs.columns
            ...
            if n_matching > 0:
                ...
                print(
                    f"  Subset metacells by {subset_col}={group_name}: {n_matching}/{mc_adata.n_obs} metacells"
                )
            else:
                print(
                    f"  WARNING: No metacells found for {subset_col}={group_name}, using all {mc_adata.n_obs} metacells"
                )
        ...
        if (
            ...
            and group_by in adata.obs.columns
            and group_name is not None
        ):
            if isinstance(group_name, str):
    ...
    print(
        f"SetDatExpr complete: {dat_expr.shape[0]} genes x {dat_expr.shape[1]} samples/cells"
    )
...
def normalize_metacells(adata: AnnData, wgcna_name: str = None) -> AnnData:
    ...
    scale_factors = np.where(
        total_counts_per_cell > 0, scale_factor / total_counts_per_cell, 0.0
    )
