...
class HDWGCNA:
    ...
    def __repr__(self):
        ...
        if 'hdWGCNA' in self.adata.uns:
            ...
            if active != 'N/A' and active in self.adata.uns['hdWGCNA']:
                ...
                if 'modules_df' in wd:
                    mods = wd['modules_df']
                    n_mods = len(set(mods['module'])) - (1 if 'grey' in set(mods['module']) else 0)
                    status.append(f"  modules: {n_mods}")
                
                if 'hMEs' in wd:
                    status.append(f"  hMEs: computed")
                
                if wd.get('kME_computed'):
                    status.append(f"  kME: computed")
    ...
    def setup_for_wgcna(self, gene_select: str = "fraction", 
                        fraction: float = 0.05,
                        n_genes: int = None,
                        genes_use: list = None,
                        wgcna_name: str = None):
        ...
        self.adata = self._core.setup_for_wgcna(
            self.adata, gene_select=gene_select, fraction=fraction,
            n_genes=n_genes, genes_use=genes_use, wgcna_name=wgcna_name
        )
        return self
    
    def set_dat_expr(self, group_name: str | list = None,
                     group_by: str = None,
                     assay: str = 'RNA',
                     layer: str = 'data',
        ...
        self.adata = self._core.set_dat_expr(
            self.adata, group_name=group_name, group_by=group_by,
            assay=assay, layer=layer, use_metacells=use_metacells,
    ...
    def metacells_by_groups(self, group_by: list | str = None,
                            reduction: str = 'pca',
                            k: int = 25,
                            max_shared: int = 10,
        ...
        self.adata = self._metacells.metacells_by_groups(
            self.adata, group_by=group_by, reduction=reduction,
            k=k, max_shared=max_shared, ident_group=ident_group,
            min_cells=min_cells, target_metacells=target_metacells,
            wgcna_name=wgcna_name
        )
        return self
    ...
    def aggregate_gene_expression(self, group_by: str | list = None,
                                  method: str = 'mean',
                                  wgcna_name: str = None):
    ...
    def test_soft_powers(self, power_range: list = None,
        ...
        self.adata = self._network.test_soft_powers(
            self.adata, power_range=power_range, network_type=network_type,
            cor_method=cor_method, wgcna_name=wgcna_name
        )
        return self
    
    def construct_network(self, power: int = None,
        ...
        **kwargs
        ...
        self.adata = self._network.construct_network(
            self.adata, power=power, tom_name=tom_name,
            network_type=network_type, tom_type=tom_type,
            tom_denom=tom_denom, minModuleSize=minModuleSize,
            deepSplit=deepSplit, pamRespectsDendro=pamRespectsDendro,
            pamStage=pamStage, mergeCutHeight=mergeCutHeight,
            wgcna_name=wgcna_name, **kwargs
        )
        return self
    ...
    def module_eigengenes(self, group_by_vars: str | list = "Sample",
                          harmonize: bool = True,
                          wgcna_name: str = None):
        ...
        self.adata = self._modules.module_eigengenes(
            self.adata, group_by_vars=group_by_vars,
            harmonize=harmonize, wgcna_name=wgcna_name
        )
        return self
    
    def module_connectivity(self, group_by: str = None,
        ...
        self.adata = self._modules.module_connectivity(
            self.adata, group_by=group_by, group_name=group_name,
            use_metacells=use_metacells, cor_method=cor_method,
            sparse=sparse, wgcna_name=wgcna_name
        )
        return self
    
    def reassign_modules(self, harmonized: bool = True,
        ...
        self.adata = self._modules.reassign_modules(
            self.adata, harmonized=harmonized,
            auto_reassign=auto_reassign, wgcna_name=wgcna_name
        )
        return self
    
    def reset_module_names(self, new_name: str = "M",
                           wgcna_name: str = None):
    ...
    def get_mes(self, harmonized: bool = True,
                wgcna_name: str = None) -> pd.DataFrame:
        """Get module eigengenes."""
        return self._modules.get_mes(self.adata, harmonized, wgcna_name)
    
    def get_hub_genes(self, n_hubs: int = 10,
                      wgcna_name: str = None) -> pd.DataFrame:
    ...
    def plot_soft_powers(self, selected_power: int = None,
                         point_size: float = 50, text_size: int = 8,
                         plot_connectivity: bool = True,
                         wgcna_name: str = None,
                         save_path: str = None):
        """Plot soft power threshold selection results."""
        return self._plotting.plot_soft_powers(
            self.adata, selected_power=selected_power, point_size=point_size,
            text_size=text_size, plot_connectivity=plot_connectivity,
            wgcna_name=wgcna_name, save_path=save_path
        )

    def module_feature_plot(self, module_names=None, reduction='umap',
                             features='hMEs', point_size=3, alpha=1.0,
                             restrict_range=True, wgcna_name=None,
                             save_path=None, ncols=3):
        """Plot module eigengenes on UMAP/tSNE."""
        return self._plotting.module_feature_plot(
            self.adata, module_names=module_names, reduction=reduction,
            features=features, point_size=point_size, alpha=alpha,
            restrict_range=restrict_range, wgcna_name=wgcna_name,
            save_path=save_path, ncols=ncols
        )

    def plot_dendrogram(self, group_labels="Module Colors", hang=0.03,
                        add_guide=True, guide_hang=0.05, main="",
                        wgcna_name=None, save_path=None):
        """Plot gene dendrogram with module colors."""
        return self._plotting.plot_dendrogram(
            self.adata, group_labels=group_labels, hang=hang,
            add_guide=add_guide, guide_hang=guide_hang, main=main,
            wgcna_name=wgcna_name, save_path=save_path
        )

    def plot_kmes(self, n_hubs=10, text_size=6, ncols=4,
                  wgcna_name=None, save_path=None):
        """Plot kME barplots per module."""
        return self._plotting.plot_kmes(
            self.adata, n_hubs=n_hubs, text_size=text_size, ncols=ncols,
            wgcna_name=wgcna_name, save_path=save_path
        )

    def module_correlogram(self, features='hMEs', exclude_grey=True,
                           method='ellipse', cmap=None,
                           wgcna_name=None, save_path=None):
        """Plot module eigengene correlation heatmap."""
        return self._plotting.module_correlogram(
            self.adata, features=features, exclude_grey=exclude_grey,
            method=method, cmap=cmap, wgcna_name=wgcna_name,
            save_path=save_path
        )

    def module_network_plot(self, n_inner=10, n_outer=15, n_conns=500,
                            mods="all", outdir="ModuleNetworks",
                            wgcna_name=None, plot_size=(6, 6),
                            edge_alpha=0.25, edge_width=1,
                            vertex_label_cex=1, vertex_size=6):
        """Generate circular network plots for each module."""
        return self._plotting.module_network_plot(
            self.adata, n_inner=n_inner, n_outer=n_outer, n_conns=n_conns,
            mods=mods, outdir=outdir, wgcna_name=wgcna_name,
            plot_size=plot_size, edge_alpha=edge_alpha, edge_width=edge_width,
            vertex_label_cex=vertex_label_cex, vertex_size=vertex_size
        )

    def generate_all_plots(self, output_dir, wgcna_name=None,
                          formats=['pdf']):
        """Generate all standard hdWGCNA plots."""
        return self._plotting.generate_all_plots(
            self.adata, output_dir=output_dir, wgcna_name=wgcna_name,
            formats=formats
        )
