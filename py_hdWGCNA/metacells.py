...
def metacells_by_groups(
    ...
    if reduction not in adata.obsm:
        ...
        raise ValueError(
            f"Reduction '{reduction}' not found. Available: {available_reductions}"
        )
    ...
    if skipped:
        print(
            f"Removing the following groups that did not meet min_cells: {', '.join(skipped)}"
        )
    ...
    mc_adata = AnnData(
        X=final_counts.copy(), var=pd.DataFrame(index=adata.var_names), obs=final_obs
    )
    ...
    print(
        f"MetacellsByGroups complete: {total_mcs} metacells created from {adata.n_obs} cells"
    )
...
def normalize_metacells(adata: AnnData, wgcna_name: str = None) -> AnnData:
    ...
    if "counts" in mc_adata.layers:
        ...
        print(
            "WARNING: No 'counts' layer found in metacell object, skipping normalization"
        )
    return adata
...
def _bootstrap_metacells(
    coords: np.ndarray, k: int, max_shared: int, target_metacells: int, max_iter: int
) -> list:
    n_cells = coords.shape[0]
