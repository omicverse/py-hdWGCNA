...
def test_soft_powers(
    ...
    **kwargs
    ...
    from .utils import compute_correlation_matrix, scale_free_fit_index_full
    ...
    print(
        f"Testing {len(power_range)} soft powers on {n_genes} genes x {n_samples} samples..."
    )
...
def construct_network(
    ...
    **kwargs
    ...
    dat_expr = wgcna_data["dat_expr"]
    gene_names = wgcna_data.get(
        "dat_expr_genes", [f"Gene_{i}" for i in range(dat_expr.shape[0])]
    )
    ...
    print(
        f"ConstructNetwork: building co-expression network with power={power}, "
    ...
    print(
        f"  Adj matrix shape: {adj_matrix.shape}, range=[{adj_matrix.min():.6f}, {adj_matrix.max():.6f}]"
    )
    ...
    non_grey = len([lb for lb in unique_labels if lb != 0])

    if non_grey == 0 and detectCutHeight is not None:
        print(
            f"  detectCutHeight={detectCutHeight} produced 0 modules, retrying with auto cutHeight..."
        )
        labels = hclust_and_cut(
            tom_dissim,
            deepSplit=deepSplit,
    ...
    colors_array = np.array([label_to_color.get(lb, "grey") for lb in labels_merged])

    modules_df = pd.DataFrame(
        {
            "gene_name": gene_names,
            "module": [f"M{lb}" if lb != 0 else "grey" for lb in labels_merged],
            "color": colors_array,
        }
    )
    ...
    _n_merged_modules = MEs_merged.shape[0]  # noqa: F841
    ...
    print(
        f"ConstructNetwork complete: {n_final_modules} co-expression modules identified"
    )
...
def _merge_via_python(
    MEs: np.ndarray, labels: np.ndarray, cut_height: float, dat_expr: np.ndarray = None
) -> tuple:
    n_modules = MEs.shape[0]
    ...
    print(
        f"  ME dissimilarity range: [{me_dissim[me_dissim > 0].min():.4f}, {me_dissim.max():.4f}]"
    )
    for i in range(me_cor.shape[0]):
        for j in range(i + 1, me_cor.shape[0]):
            print(
                f"    Module {i + 1} vs Module {j + 1}: cor={me_cor[i, j]:.4f}, dissim={me_dissim[i, j]:.4f}"
            )
    ...
    for old_id in unique_old:
        ...
        for other_old in unique_old:
            ...
            if other_me_idx is not None and other_me_idx < len(merged_labels):
                if (
                    merged_labels[other_me_idx] == target_cluster
                    and other_old < min_old_in_cluster
                ):
        ...
        if dat_expr is not None:
            ...
            if len(gene_indices) > 0:
                ...
                if module_expr.shape[0] > 1:
                    mod_centered = module_expr - np.mean(
                        module_expr, axis=1, keepdims=True
                    )
            ...
            old_modules_in_cluster = [
                old_id
                for old_id, new_id_map in old_to_new.items()
                if new_id_map == mod_id
            ]
            me_indices_for_this_cluster = [
                label_to_me_index[old_id]
                for old_id in old_modules_in_cluster
                if old_id in label_to_me_index
            ]
...
def _generate_module_colors(unique_labels: np.ndarray) -> dict:
    ...
    sorted_labels = sorted([lb for lb in unique_labels if lb != 0])
