...
WGCNA_COLORS = [
    'turquoise', 'blue', 'brown', 'yellow', 'green', 'grey60',
    'magenta', 'purple', 'pink', 'black', 'red', 'cyan',
    'greenyellow', 'tan', 'salmon', 'skyblue', 'orange',
    'midnightblue', 'lightyellow', 'lightcyan', 'coral'
]
...
_R_COLOR_MAP = {
    'grey90': '#E6E6E6', 'grey80': '#CCCCCC', 'grey70': '#B3B3B3',
    'grey60': '#999999', 'grey50': '#808080', 'grey40': '#666666',
    'grey30': '#4D4D4D', 'grey20': '#333333', 'grey10': '#1A1A1A',
    'greenyellow': '#ADFF2F', 'midnightblue': '#191970',
    'lightyellow': '#FFFFE0', 'lightcyan': '#E0FFFF',
}
...
def plot_soft_powers(adata: AnnData,
                     selected_power: int = None,
                     point_size: float = 50,
                     text_size: int = 8,
                     plot_connectivity: bool = True,
                     wgcna_name: str = None,
                     save_path: str = None) -> Union[plt.Figure, List[plt.Figure]]:
    """
    ...
    if isinstance(pt, pd.DataFrame):
        ...
        median_k = np.asarray(pt.get('median.k.', pt.get('median_k', np.zeros(len(powers)))))
    ...
    n_plots = 4 if plot_connectivity else 1
    ...
    colors = ['white' if p != selected_power else 'black' for p in powers]
    ax1.scatter(powers, sft_rsq, s=point_size, c='none', edgecolors='black', linewidths=0.8, zorder=5)
    ax1.scatter([selected_power], [sft_r_val], s=point_size*1.3, c='black', zorder=6)

    for i, (p, r) in enumerate(zip(powers, sft_rsq)):
        ax1.text(p, r, str(int(p)), ha='center', va='center',
                fontsize=text_size, fontweight='bold' if p == selected_power else 'normal',
                color='white' if p == selected_power else 'black', zorder=7)
    ...
    if plot_connectivity:
        ...
        ax2.axvline(x=selected_power, color='black', linestyle='--', linewidth=1, alpha=0.7)
        ax2.scatter(powers, mean_k, s=point_size, c='none', edgecolors='black', linewidths=0.8)
        ax2.scatter([selected_power], [mean_k_val], s=point_size*1.3, c='black')
        for p, k in zip(powers, mean_k):
            ax2.text(p, k, str(int(p)), ha='center', va='center',
                    fontsize=text_size, color='white' if p == selected_power else 'black')
        ...
        ax3.axvline(x=selected_power, color='black', linestyle='--', linewidth=1, alpha=0.7)
        ax3.scatter(powers, median_k, s=point_size, c='none', edgecolors='black', linewidths=0.8)
        ax3.scatter([selected_power], [med_k_val], s=point_size*1.3, c='black')
        for p, k in zip(powers, median_k):
            ax3.text(p, k, str(int(p)), ha='center', va='center',
                    fontsize=text_size, color='white' if p == selected_power else 'black')
        ...
        ax4.axvline(x=selected_power, color='black', linestyle='--', linewidth=1, alpha=0.7)
        ax4.scatter(powers, max_k, s=point_size, c='none', edgecolors='black', linewidths=0.8)
        ax4.scatter([selected_power], [max_k_val], s=point_size*1.3, c='black')
        for p, k in zip(powers, max_k):
            ax4.text(p, k, str(int(p)), ha='center', va='center',
                    fontsize=text_size, color='white' if p == selected_power else 'black')
...
def module_feature_plot(adata: AnnData,
                        ...
                        save_path: str = None,
                        ncols: int = 3,
                        order_points: bool = True) -> Union[plt.Figure, Dict[str, plt.Figure]]:
    """
    ...
    me_key = {'hMEs': 'hMEs', 'MEs': 'MEs', 'scores': 'module_scores',
              'average': 'avg_module_expr'}.get(features, 'hMEs')
    MEs = wd.get(me_key)
    ...
    for idx, cur_mod in enumerate(module_names):
        ...
        ax.scatter(plot_df['x'], plot_df['y'],
                   c=plot_df['val'], cmap=cmap_custom, s=point_size, alpha=alpha,
...
def plot_dendrogram(adata: AnnData,
    ...
    if Z is None:
        ...
        if tom_dissim is not None:
            ...
            raise ValueError("No linkage matrix or TOM dissimilarity found. Run construct_network() first.")
    ...
    if gene_names is None:
        gene_names = modules_df['gene_name'].tolist() if 'gene_name' in modules_df.columns else list(range(len(modules_df)))
    ...
    for i in range(Z.shape[0]):
        ...
        ax_dendro.plot([left_x_val, left_x_val], [left_y, merge_h], color='#555555', linewidth=0.6)
        ax_dendro.plot([right_x_val, right_x_val], [right_y, merge_h], color='#555555', linewidth=0.6)
        ax_dendro.plot([left_x_val, right_x_val], [merge_h, merge_h], color='#555555', linewidth=0.6)
    ...
    ax_color.set_xticks([])
    ax_color.set_yticks([])
    ax_color.set_ylabel(group_labels, fontsize=11, rotation=0, ha='right', va='center', labelpad=3)
...
def plot_kmes(adata: AnnData,
    ...
    if 'kME' in modules_df.columns:
        ...
        kme_cols = [c for c in modules_df.columns
    ...
    for idx, cur_mod in enumerate(unique_mods):
        ...
        ax.barh(y_pos, display_genes[kme_col].values,
               color=cur_color, edgecolor=cur_color, height=0.8)

        ax.set_yticks(y_pos)
        ax.set_yticklabels(display_genes['gene_name'].values if 'gene_name' in display_genes.columns
                           else display_genes.index, fontsize=text_size, style='italic')
        ax.set_title(cur_mod, fontsize=11, fontweight='bold')
...
def module_correlogram(adata: AnnData,
                       features: str = 'hMEs',
    ...
    r_cmap = LinearSegmentedColormap.from_list('seagreen_white_darkorchid', r_colors, N=256)
    ...
    for i in range(n):
        for j in range(n):
            ...
            if method == 'ellipse':
                ...
                ellipse = Ellipse((cx, cy), width=w, height=h, angle=angle,
                ...
                ax.text(cx, cy, f'{r_val:.2f}', ha='center', va='center',
                       fontsize=max(6, 10 - n // 3), color=txt_color, zorder=4)

    for i in range(n):
        ax.text(i, -0.6, clean_names[i], ha='center', va='bottom',
               fontsize=max(7, 10 - n // 3), rotation=tl_srt, zorder=6)
        ax.text(i - 0.55, i, clean_names[i], ha='right', va='center',
               fontsize=max(7, 10 - n // 3), zorder=6)
...
def module_network_plot(adata: AnnData,
                         n_inner: int = 10,
                         n_outer: int = 15,
                         n_conns: int = 500,
                         ...
                         vertex_size: float = 6) -> Dict[str, plt.Figure]:
    """
    ...
    for cur_mod in plot_mods:
        ...
        cur_genes = mod_hub_df['gene_name'].values if 'gene_name' in mod_hub_df.columns else mod_hub_df.index.values
        ...
        for k in pos:
            pos[k] = (pos[k][0] + rng.uniform(-jitter_amount, jitter_amount),
        ...
        nx.draw_networkx_nodes(G, pos, ax=ax, node_color=cur_color,
                               node_size=vertex_size * 20, edgecolors='black',
                               linewidths=0.5)
...
def hub_gene_network_plot(adata: AnnData,
                           mods: Union[str, List[str]] = "all",
                           n_hubs: int = 6,
                           n_other: int = 3,
    ...
    for cur_mod in plot_mods:
        ...
        hub_genes = mod_df['gene_name'].values if 'gene_name' in mod_df.columns else mod_df.index.values
    ...
    for cur_mod in plot_mods:
        ...
        if len(mod_non_hub) > 0:
            ...
            sampled = mod_non_hub.sample(n=n_sample, replace=True, random_state=rng_other.integers(0, 2**31))
    ...
    gene_to_node_idx = {g: i for i, g in enumerate(valid_genes)}
    ...
    for mc, indices in mod_gene_indices.items():
        ...
        n_intra_keep = min(len(pair_vals), max(3 * n_genes_in_mod, n_genes_in_mod * (n_genes_in_mod - 1) // 2))
    ...
    for i in range(n_valid):
        for j in range(i + 1, n_valid):
            ...
            c1 = _to_mpl_color(r1.get('color', 'grey')) if isinstance(r1, dict) else 'grey'
            c2 = _to_mpl_color(r2.get('color', 'grey')) if isinstance(r2, dict) else 'grey'
            if c1 != c2:
                inter_pair_vals.append((subset_TOM[i, j], i, j))

    inter_pair_vals.sort(key=lambda x: -x[0])
    n_inter_keep = min(len(inter_pair_vals), max(3 * len(mod_gene_indices), len(mod_gene_indices) + 5))
    ...
    edge_df['color_final'] = edge_df.apply(
        lambda row: to_rgba(_to_mpl_color(row['color_raw']), alpha=float(row['value_scaled'])),
        axis=1
    )
    ...
    for u, v in G.edges():
        ...
        ax.plot(
            ...
            color=color, alpha=alpha_val, linewidth=0.5, zorder=1
        )
    ...
    nx.draw_networkx_nodes(G, pos, ax=ax, node_color=node_colors, node_size=node_sizes,
                            edgecolors=node_colors, linewidths=0.5)
    nx.draw_networkx_labels(G, pos, labels=labels, ax=ax, font_size=vertex_label_cex * 8,
                            font_color='black')
...
def module_umap_plot(adata: AnnData,
                      sample_edges: bool = True,
                      edge_prop: float = 0.2,
                      label_hubs: int = 5,
                      edge_alpha: float = 0.25,
    ...
    umap_genes = umap_df['gene'].values if 'gene' in umap_df.columns else umap_df.index.values
    n_genes = len(umap_genes)
    ...
    selected = pd.DataFrame({
        ...
        'UMAP1': umap_df['UMAP1'].values if 'UMAP1' in umap_df.columns else umap_df.values[:, 0],
        'UMAP2': umap_df['UMAP2'].values if 'UMAP2' in umap_df.columns else umap_df.values[:, 1],
        'hub': umap_df['hub'].values if 'hub' in umap_df.columns else 'other',
        'kME': umap_df['kME'].values if 'kME' in umap_df.columns else [gene_name_to_kme.get(g, 0) for g in umap_genes],
        'color': [gene_name_to_color.get(g, 'grey') for g in umap_genes],
        'module': [gene_name_to_module.get(g, 'grey') for g in umap_genes],
    })
    selected['label'] = selected['gene_name'].apply(lambda x: x if x in hub_labels else '')
    selected['fontcolor'] = selected['color'].apply(lambda c: '#808080' if c == 'black' else 'black')
    ...
    def _to_mpl_color(c):
        color_map = {
            'grey90': '#E6E6E6', 'grey60': '#999999',
            'greenyellow': '#ADFF2F', 'lightyellow': '#FFFFE0',
            'lightcyan': '#E0FFFF', 'midnightblue': '#191970',
            'skyblue': '#87CEEB', 'darkgrey': '#A9A9A9',
        }
        if c in color_map:
            return color_map[c]
        try:
    ...
    edge_df['color_final'] = edge_df.apply(
        lambda row: to_rgba(
            ...
            alpha=float(row['value_scaled']) / 2 if row['color_raw'] == 'grey90' else float(row['value_scaled'])
        ),
        axis=1
    )
    ...
    if return_graph:
        try:
            ...
            for _, row in edge_df.iterrows():
                G.add_edge(row['gene1'], row['gene2'], weight=row['value'], color=row['color_final'])
            return G
        except ImportError:
            return None
    ...
    if edge_lines:
        lc = LineCollection(edge_lines, colors=edge_colors_list, linewidths=0.5, alpha=edge_alpha)
    ...
    ax.scatter(
        coords[:, 0], coords[:, 1],
        s=node_sizes, c=node_colors_rgba,
        edgecolors=node_framecolors, linewidths=0.5,
        zorder=3
    )
    ...
    if len(label_rows) > 0:
        ...
        try:
            ...
            adjust_text(
                ...
                ax=ax,
                arrowprops=dict(
                    arrowstyle='-', color='#555555', lw=0.5,
...
def plot_dmes_volcano(adata: AnnData,
                       dme_df: pd.DataFrame,
    ...
    lowest_nonzero = df[df['p_val'] > 0]['p_val'].min() if any(df['p_val'] > 0) else 1e-10
    ...
    df['anno'] = df.apply(lambda r: r['module'] if r.get('p_val_adj', r['p_val']) < 0.05 else '', axis=1)
    ...
    ax.scatter(df['avg_log2FC'], -np.log10(df['p_val']),
               s=mod_point_size**2 * 5, c=colors, edgecolors='black', linewidths=0.5, zorder=5)

    if plot_labels:
        sig = df[df['anno'] != '']
        for _, row in sig.iterrows():
            ax.annotate(row['anno'], (row['avg_log2FC'], -np.log10(row['p_val'])),
...
def plot_dmes_lollipop(adata: AnnData,
                        dme_df: pd.DataFrame,
                        group_by: str = None,
    ...
    comparisons = df[group_by].unique() if group_by and group_by in df.columns \
    ...
    for comp in comparisons:
        cdf = df[df[group_by] == comp] if group_by and group_by in df.columns else df.copy()
        ...
        for i, (_, row) in enumerate(cdf.iterrows()):
            ...
            ax.scatter(fc, i, s=np.log(n_g+1)*30, c=mod_colors.get(row['module'], '#0072B2'),
                      edgecolors='black', linewidths=0.5, zorder=5)
            ax.plot([0, fc], [i, i], color=colors[i], linewidth=0.8, alpha=0.4, zorder=3)
            ax.scatter(fc, i, marker=shape, s=40, c='white' if shape=='o' else 'black',
...
def plot_module_trait_correlation(adata: AnnData,
                                    high_color: str = 'red',
    ...
    modules_df = wd.get('modules_df')
    mod_colors = modules_df.drop_duplicates('module')[['module', 'color']].set_index('module')
    ...
    for tname, cor_mat in temp['cor'].items():
        ...
        if label and pval_mat is not None:
            ...
            if label_symbol == 'stars':
                ...
                cor_df['sig'] = cor_df.apply(
                    lambda r: format(r['pval'], f'.{text_digits}f') if r['pval'] < 0.05 else '',
                    axis=1)
        ...
        cmap = LinearSegmentedColormap.from_list('trait_cmap', [low_color, mid_color, high_color])

        pivot = cor_df.pivot(index='Trait' if False else 'dummy', columns='Module', values='cor')
        if pivot.shape[0] == 0:
            pivot = pd.DataFrame({m: [0] for m in mods_plot})

        im = ax.imshow(pivot.values, cmap=cmap, aspect='auto', vmin=-plot_max, vmax=plot_max)

        for i, mod in enumerate(mods_plot):
            mod_data = cor_df[cor_df['Module'] == mod]
            for j, (_, row) in enumerate(mod_data.iterrows()):
                val = row['cor']
                clr = 'white' if abs(val) > plot_max*0.5 else 'black'
                if label and 'sig' in cor_df.columns:
                    txt = row.get('sig', '')
                    if txt:
                        ax.text(i, j, txt, ha='center', va='center', fontsize=text_size,
    ...
    if combine and len(plot_list) > 1:
        ...
        outer_gs = gridspec.GridSpec(n_plots + 1, 1, height_ratios=[1]*n_plots + [0.15], hspace=0.05)

        for i, (tname, fig) in enumerate(plot_list.items()):
            sub_ax = fig2.add_subplot(outer_gs[i])
            old_ax = fig.axes[0]
            im2 = old_ax.images[0]
            sub_ax.imshow(im2.get_array(), cmap=im2.cmap, aspect='auto', vmin=im2.get_clim()[0], vmax=im2.get_clim()[1])
            sub_ax.set_title(tname, fontsize=10, fontweight='bold')
            sub_ax.set_xticks(old_ax.get_xticks())
            sub_ax.set_xticklabels(old_ax.get_xticklabels(), rotation=45, ha='right', fontsize=7)
            sub_ax.set_yticks([])

        fig2.suptitle('Module-Trait Correlation', fontsize=12, fontweight='bold', y=0.995)
        plt.tight_layout(pad=0.5)
...
def enrichr_bar_plot(enrichr_results: Union[pd.DataFrame, Dict[str, pd.DataFrame]],
                      top_n: int = 10,
    ...
    fig, axes = plt.subplots(len(groups), 1, figsize=(8, max(4, len(groups) * 3)),
                             squeeze=False)

    for i, grp in enumerate(groups):
        ax = axes[i][0]
        grp_data = df[df[group_by] == grp].nlargest(top_n, columns='combined_score') \
                   if 'combined_score' in df.columns else df[df[group_by] == grp].head(top_n)

        y_pos = np.arange(len(grp_data))
        ax.barh(y_pos, -np.log10(grp_data['pvalue']), color=color, edgecolor='black', linewidth=0.5)
        ax.set_yticks(y_pos)
...
def enrichr_dot_plot(enrichr_results: Union[pd.DataFrame, Dict[str, pd.DataFrame]],
                      top_n: int = 15,
                      group_by: str = 'database',
    ...
    fig, axes = plt.subplots(len(groups), 1, figsize=(9, max(4, len(groups) * 3.5)),
                             squeeze=False)

    for i, grp in enumerate(groups):
        ax = axes[i][0]
        grp_data = df[df[group_by] == grp].copy()
        grp_data = grp_data.nlargest(top_n, columns='combined_score') \
                   if 'combined_score' in df.columns else grp_data.head(top_n)

        if len(grp_data) == 0:
            continue

        grp_data[size_col] = pd.to_numeric(grp_data[size_col], errors='coerce').fillna(0)
        grp_data['n_genes'] = pd.to_numeric(grp_data['n_genes'], errors='coerce').fillna(1)
        ...
        scatter = ax.scatter(grp_data['gene_ratio'], -np.log10(grp_data['pvalue']),
                            s=sizes, c='#0072B2', alpha=0.6, edgecolors='black', linewidths=0.5)

        labels = grp_data['term'].str[:45]
        for j, (_, row) in enumerate(grp_data.iterrows()):
            ax.annotate(labels.iloc[j], (row['gene_ratio'], -np.log10(row['pvalue'])),
...
def plot_module_preservation(preserv_df: pd.DataFrame,
    ...
    colors_map = {
        'highly preserved': '#009E73', 'moderately preserved': '#56B4E9',
        'weakly preserved': '#F0E442', 'non-preserved': '#D55E00', 'error': 'grey'
    }
    bar_colors = [colors_map.get(p, 'grey') for p in preserv_df['preservation']]

    ax = axes[0]
    y_pos = np.arange(len(preserv_df))
    ax.barh(y_pos, preserv_df['Zsummary'], color=bar_colors, edgecolor='black', linewidth=0.5)
    ax.axvline(x=z_thresholds['weak'], color='#D55E00', linestyle='--', alpha=0.7, linewidth=1)
    ax.axvline(x=z_thresholds['moderate'], color='#0072B2', linestyle='--', alpha=0.7, linewidth=1)
    ax.axvline(x=z_thresholds['high'], color='#009E73', linestyle='--', alpha=0.7, linewidth=1)
    ax.set_yticks(y_pos)
    ...
    ax2.barh(y_pos2, valid_mr['medianRank'], color=[bar_colors[i] for i, row in preserv_df.iterrows() if row['medianRank'] == row['medianRank']],
...
def compute_module_umap(adata: AnnData,
                        n_hubs: int = 10,
                        exclude_grey: bool = True,
    ...
    print(f"compute_module_umap complete: {plot_df.shape[0]} genes, "
...
def module_dot_plot(adata: AnnData,
                    features: str = 'hMEs',
    ...
    if isinstance(MEs, np.ndarray):
        mod_names = wd.get('module_names', [f'M{i}' for i in range(MEs.shape[1])])
        MEs = pd.DataFrame(MEs, columns=mod_names, index=adata.obs_names[:MEs.shape[0]])
    ...
    cell_groups = adata.obs[group_by].astype(str).values
    ...
    avg_exp = pd.DataFrame(np.zeros((n_groups, n_mods)),
                           index=unique_groups, columns=mods)
    pct_exp = pd.DataFrame(np.zeros((n_groups, n_mods)),
                           index=unique_groups, columns=mods)
    ...
    scatter = ax.scatter(
        plot_df['module'], plot_df['group'],
        c=plot_df['avg_scaled'], cmap=cmap, norm=norm_color,
        s=sizes, edgecolors='none', linewidths=0.5, zorder=3
    )
    ...
    ax.set_xticks(range(n_mods))
    ax.set_xticklabels(mods, fontsize=10, ha='right' if rotate_x_labels else 'center',
    ...
    for pv, sz in zip(pct_vals, legend_sizes):
        legend_elements.append(
            plt.scatter([], [], s=sz, c='grey', edgecolors='black',
                       linewidths=0.5, label=f'{int(pv*100)}%')
        )
...
def module_radar_plot(adata: AnnData,
                      group_by: str = None,
                      barcodes: list = None,
                      features: str = 'hMEs',
                      exclude_grey: bool = True,
                      ...
                      combine: bool = True,
                      wgcna_name: str = None,
                      save_path: str = None) -> Union[plt.Figure, Dict[str, plt.Figure]]:
    """
    ...
    def _draw_single_radar(mod_name, values, color, ax):
        ...
        for i, (angle, cluster) in enumerate(zip(angles, clusters)):
            ax.plot([0, max_val * 1.05 * np.cos(angle)],
            ...
            if rot > 180:
                rot -= 360
            ax.text(lx, ly, cluster, ha='center', va='center',
                    fontsize=axis_label_size, rotation=rot, zorder=5)
        ...
        for i, (gv, gl) in enumerate(zip(grid_values, grid_levels)):
            ax.text(0.02, gv, f'{gl:.0%}',
                    fontsize=grid_label_size, color='#999999', ha='left', va='center', zorder=5)
    ...
    if combine:
        ...
        fig_list = {}
        for mod in mods:
            fig_single, ax_single = plt.subplots(figsize=(3, 3), subplot_kw=dict(polar=False))
...
def generate_all_plots(adata: AnnData,
                       output_dir: str,
                       wgcna_name: str = None,
                       formats: List[str] = ['pdf']) -> Dict[str, str]:
    """
    ...
    try:
        f = os.path.join(output_dir, 'soft_powers')
        for fmt in formats:
            plot_soft_powers(adata, wgcna_name=wgcna_name,
                           save_path=f".{fmt}")
    ...
    try:
        f = os.path.join(output_dir, 'dendrogram')
        for fmt in formats:
            plot_dendrogram(adata, wgcna_name=wgcna_name,
                          save_path=f".{fmt}")
    ...
    try:
        f = os.path.join(output_dir, 'module_feature_plot')
        for fmt in formats:
            module_feature_plot(adata, wgcna_name=wgcna_name,
                              save_path=f".{fmt}")
    ...
    try:
        f = os.path.join(output_dir, 'kme_bars')
        for fmt in formats:
            plot_kmes(adata, wgcna_name=wgcna_name,
                     save_path=f".{fmt}")
    ...
    try:
        f = os.path.join(output_dir, 'correlogram')
        for fmt in formats:
            module_correlogram(adata, wgcna_name=wgcna_name,
                             save_path=f".{fmt}")
