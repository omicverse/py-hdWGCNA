...
def project_modules(adata: AnnData,
    ...
    if isinstance(hMEs_source, np.ndarray):
        mod_names = wd.get('module_names', [f'M{i}' for i in range(hMEs_source.shape[1])])
    ...
    if source_hvg is None:
        ...
        if source_hvg is None or len(source_hvg) == 0:
            source_hvg = [g for g in adata.var_names if g in modules_df['gene_name'].values]
    ...
    if len(shared_genes) < 20:
        raise ValueError(f"Only {len(shared_genes)} shared genes between source and target (min 20).")

    print(f"Projecting modules onto target ({target_adata.n_obs} cells, {len(shared_genes)} shared genes)")
    ...
    for cur_mod in unique_mods:
        ...
        src_pc1 = np.zeros(src_scaled.shape[0])
        ...
        if pc1_norm > 0:
            pc1 = pc1 / pc1_norm
        src_pc1 = src_scaled[:, src_mod_idx] @ pc1
...
def module_preservation(adata_ref: AnnData,
                         adata_test: AnnData,
    ...
    for cur_mod in unique_mods:
        ...
        mod_genes = mod_row['gene_name'].values.tolist() if 'gene_name' in mod_row.columns else mod_row.index.tolist()
        mod_genes_common = [g for g in mod_genes if g in common_genes]

        if len(mod_genes_common) < 5:
            preservation_stats.append({
                'module': cur_mod, 'Zsummary': np.nan, 'medianRank': np.nan,
                'preservation': 'error', 'n_genes': len(mod_genes_common)
            })
            continue
        ...
        obs_median_rank = np.median([np.median(ref_TOM_sub[i, mod_local_idx]) for i in mod_local_idx])
        ...
        for perm_i in range(n_permutations):
            ...
            perm_med_ranks[perm_i] = np.median([np.median(ref_TOM_sub[p, perm_indices]) for p in perm_indices])
        ...
        medianRank_Z = (obs_median_rank - perm_mr_mean) / perm_mr_std if perm_mr_std > 0 else 0
        ...
        preservation_stats.append({
            'module': cur_mod, 'Zsummary': round(Zsummary, 4), 'medianRank': round(obs_median_rank, 4),
            'preservation': preserv_cat, 'n_genes': len(mod_genes_common)
        })
