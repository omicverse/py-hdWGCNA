...
def check_wgcna_name(adata, wgcna_name):
    ...
    if "hdWGCNA" not in adata.uns or wgcna_name not in adata.uns["hdWGCNA"]:
        raise ValueError(
            f"hdWGCNA experiment '{wgcna_name}' not found in adata.uns['hdWGCNA']"
        )
    return wgcna_name
...
def soft_threshold(
    cor_matrix: np.ndarray, power: int, network_type: str = "signed"
) -> np.ndarray:
    """
...
def compute_tom(
    adj_matrix: np.ndarray, tom_type: str = "signed", tom_denom: str = "min"
) -> np.ndarray:
    __n = adj_matrix.shape[0]  # noqa: F841
...
def compute_module_eigengenes(
    expr_mat: np.ndarray, module_labels: np.ndarray, exclude_grey: bool = True
) -> tuple:
    """
    ...
    for i, mod in enumerate(unique_modules):
        ...
        denom = np.sqrt(np.sum(avg_expr_centered**2) + 1e-30) * np.sqrt(
...
def compute_kme(
    expr_mat: np.ndarray, MEs: np.ndarray, method: str = "pearson"
) -> np.ndarray:
...
def scale_free_fit_index(
    k: np.ndarray, power: int, nBreaks: int = 10, removeFirst: bool = False
) -> float:
    result = scale_free_fit_index_full(k, nBreaks, removeFirst)
    return result["R2"]


def scale_free_fit_index_full(
    k: np.ndarray, nBreaks: int = 10, removeFirst: bool = False
) -> dict:
    ...
    try:
        ...
        n_obs = len(log_dk)
        adj_r_sq = (
            1.0 - (ss_res2 / ss_tot2) * (n_obs - 1) / (n_obs - n_params)
            if ss_tot2 > 0 and n_obs > n_params
            else 0.0
        )
...
def load_r_outputs(data_dir):
    ...
    kme_cols = [
        c
        for c in r_kme.columns
        if c.lower().startswith("kme") and c.lower() != "module"
    ]
    rename_map = {}
    for c in kme_cols:
        mod_name = c
...
def load_test_data(data_dir):
    ...
    adata = anndata.AnnData(
        X=expr_df.T.values, obs=meta_df, var=pd.DataFrame(index=expr_df.index)
    )
...
def benchmark_compare(adata, r_outputs, wgcna_name="hdWGCNA"):
    ...
    if py_sft is not None:
        if isinstance(py_sft, pd.DataFrame):
            ...
            if "Power" in py_sft_df.columns and "SFT.R.sq" in r_power.columns:
                merged = pd.merge(
                    r_power, py_sft_df, on="Power", suffixes=("_R", "_Py"), how="inner"
                )
                ...
                if len(sq_r) > 0 and len(sq_py) > 0:
                    ...
                    if len(valid) >= 2:
                        ...
                        results["power"] = {
                            "r": np.nan,
                            "error": f"insufficient overlap: {len(valid)}",
                        }
                else:
                    results["power"] = {
                        "r": np.nan,
                        "error": f"SFT columns not found: R_cols={[c for c in merged.columns if 'SFT' in c]}",
                    }
            else:
                results["power"] = {
                    "r": np.nan,
                    "error": f"Py power_table cols: {list(py_sft_df.columns[:5])}",
                }
        else:
            results["power"] = {
                "r": np.nan,
                "error": f"unexpected type: {type(py_sft).__name__}",
            }
    else:
    ...
    if py_mods is not None:
        if isinstance(py_mods, pd.DataFrame):
            mods_series = (
                py_mods.set_index(py_mods.columns[0]).iloc[:, 0]
                if py_mods.shape[1] >= 2
                else pd.Series(py_mods.iloc[:, 0].values)
            )
        ...
        if len(common_genes) > 0:
            ...
            try:
                ...
                results["modules_ari"] = round(
                    _ari_fn(r_labels.astype(str), py_labels.astype(str)), 4
                )
                results["modules_ari_binary"] = round(_ari_fn(r_binary, py_binary), 4)
                results["modules_nmi"] = round(
                    _nmi_fn(r_labels.astype(str), py_labels.astype(str)), 4
                )
        ...
        if n_pairs > 0:
            ...
            for r_mod_name in r_non_grey:
                ...
                for py_mod_name in py_non_grey_cols:
                    ...
                    try:
                        rv, pv = _pearsonr(
                            r_vals.values[:n_min], py_vals.values[:n_min]
                        )
            ...
            results["hmes"] = {}
            results["hmes_error"] = (
                f"no non-grey pairs: R={len(r_non_grey)} Py={len(py_non_grey_cols)}"
            )
    else:
        results["hmes"] = {}
        missing = []
    ...
    if r_kme is not None and len(r_kme) > 0 and py_kme_data is not None:
        r_kme_non_grey = [
            c
            for c in r_kme.columns
            if c.startswith("kME_") and "grey" not in str(c).lower()
        ]
        ...
        if isinstance(py_kme_data, np.ndarray):
            ...
            py_kme_non_grey = [
                c
                for c in py_kme_data.columns
                if str(c).lower() != "grey" and str(c).lower() != "0"
            ]
        elif isinstance(py_kme_data, dict):
            py_kme_non_grey = [
                k.replace("kME_", "")
                for k in py_kme_data.keys()
        ...
        if n_kme_pairs > 0 and len(common_genes_kme) >= 3:
            for r_col in r_kme_non_grey:
                matched = [
                    g
                    for g in common_genes_kme
                    if g in r_kme.index and g in adata.var_names
                ]
                if len(matched) < 3:
                    continue
                ...
                for py_mod_id in py_kme_non_grey:
                    if isinstance(py_kme_data, np.ndarray):
                        ...
                        col_idx = (
                            py_kme_non_grey.index(py_mod_id)
                            if isinstance(py_mod_id, str)
                            else py_mod_id
                        )
                        ...
                        py_raw = py_kme_data.get(py_mod_id) or py_kme_data.get(
                            f"kME_{py_mod_id}"
                        )
                        if py_raw is None:
                            continue
                        py_vals = pd.Series(
                            py_raw[: len(matched)], index=matched
                        ).dropna()
                    else:
                        continue
                    ...
                    if len(shared_idx) < 3:
                        continue
                    r_clean = pd.to_numeric(
                        r_vals.loc[shared_idx], errors="coerce"
                    ).dropna()
                    p_clean = pd.to_numeric(
                        py_vals.loc[shared_idx], errors="coerce"
                    ).dropna()
                    ...
                    try:
                        rv, pv = _pearsonr(
                            r_clean[common].values, p_clean[common].values
                        )
                        if abs(rv) > abs(best_r):
                            best_r, best_n, best_py = (
                                rv,
                                int(len(common)),
                                str(py_mod_id),
                            )
                ...
                if best_py is not None and abs(best_r) > 0:
                    kme_results[f"{r_col}_vs_{best_py}"] = {
                        "r": round(best_r, 4),
                        "n": best_n,
                        "R_col": r_col.replace("kME_", ""),
                        "Py_module": best_py,
                    }
            results["kme"] = kme_results
            results["kme_pairs"] = n_kme_pairs
            results["kme_common_genes"] = (
                len(matched) if "matched" in dir() else len(common_genes_kme)
            )
        else:
            results["kme"] = {}
            results["kme_error"] = (
                f"no pairs or too few: pairs={n_kme_pairs}, genes={len(common_genes_kme)}"
            )
    else:
        results["kme"] = {}
        missing_k = []
...
def format_benchmark_report(results):
    ...
    if hmes:
        ...
        for key, m in hmes.items():
            ...
            lines.append(
                f"    {r_m:>12s} vs {p_m:<12s}:  r = {m['r']:.4f}  (n={m['n']})"
            )
    ...
    if kme:
        ...
        for key, m in kme.items():
            ...
            lines.append(
                f"    {r_c:>12s} vs {p_k:<12s}:  r = {m['r']:.4f}  (n={m['n']})"
            )
